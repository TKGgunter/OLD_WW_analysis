  ...
  _algo(algo), _type(type), _errType(errType), _mu(mu)
{
  ...
  {
    const char *cd = "/home/gunter/WW_analysis/production/Analysis_13TeV/tools/jecsys/CondFormats/JetMETObjects/data";
...
void JECUncertainty::_InitL1() {
  ...
  string directory = "/home/gunter/WW_analysis/production/Analysis_13TeV/tools/jecsys/CondFormats/JetMETObjects/data/";
...
void JECUncertainty::_InitJEC() {
  ...
  { // RunBCDEFGH (all 2016)
    ...
    //s = Form("%sSummer16_03Feb2017BCDEFGH_V3_DATA_L2L3Residual_%s.txt",d,a); // custom 03FebV3 <= Why is this offical if it's custom! SMH DUMB
    s = Form("%sSummer16_03Feb2017H_V3_DATA_L2L3Residual_%s.txt",d,a);
    if (debug) cout << s << endl << flush;
    JetCorrectorParameters *l2l3res = new JetCorrectorParameters(s);
...
void JECUncertainty::_InitL2Res() {
  ...
  }
  {
    ...
    s = Form("%sSpring16_25ns_MPF_LOGLIN_L2Residual_pythia8_v4_%s.txt",d,a); // THOTH =>This is just to get shit on the screen; will need to get herwig file
    if (debug) cout << s << endl << flush;
    JetCorrectorParameters *l2l3res = new JetCorrectorParameters(s);
    vector<JetCorrectorParameters> v;
    v.push_back(*l2l3res);
...
//double JECUncertainty::Uncert(const double pTprime, const double eta) {
py::list jecUncertainties(py::list ptlist, py::list etalist, JECUncertainty& jec_obj){

  py::list list_;

  if(ptlist.size() != etalist.size()){
    printf("pt list and eta list are not the same size!");
    return list_;
  }

  for(int i =0; i < ptlist.size(); i++){
    list_.append(jec_obj.Uncert(ptlist[i].cast<float>(), etalist[i].cast<float>())); 
  }
...
py::list jerUncertainties(py::list ptlist, py::list etalist, JECUncertainty& jec_obj){
  ...
  for(int i =0; i < ptlist.size(); i++){
    list_.append(jec_obj._RelativeJER(ptlist[i].cast<float>(), etalist[i].cast<float>())); 
  }
...
PYBIND11_MODULE(JECUncertainty, m)
{

    m.doc() = "JEC uncertainty";
    m.def("test", &test, "This is a test to test the functionality of pybind11");

  
    py::class_<jec::ErrorTypes>(m, "ErrorTypes") 
      .def(py::init<unsigned long int , unsigned long int >())
      ;



  /* //In case we need to add more JetAlgo named constants
   ...
   */
    ...
    py::enum_<jec::JetAlgo>(m, "JetAlgo")
      .value("AK4PF",       jec::JetAlgo::AK4PF)
      .value("AK4PFchs",    jec::JetAlgo::AK4PFchs)
      .value("AK4PFpuppi",  jec::JetAlgo::AK4PFpuppi)
      .value("AK4CALO",     jec::JetAlgo::AK4CALO)
      .value("AK5PF",       jec::JetAlgo::AK5PF)
      /*.value("", jec::JetAlgo::)// If we ever need more place here
      .value("",   jec::JetAlgo::)
      .value("",   jec::JetAlgo::)
      .value("",   jec::JetAlgo::)
      .value("",   jec::JetAlgo::)*/ 
    ...
    py::enum_<jec::DataType>(m, "DataType")
      .value("DATA", jec::DataType::DATA)
      .value("MC",   jec::DataType::MC)
      .value("PY",   jec::DataType::PY)
      .value("HW",   jec::DataType::HW)
      ;

    m.attr("kData") = jec::kData;
    m.attr("kMC") = jec::kMC;
    
    py::class_<JECUncertainty>(m, "JECUncertainty") 
      .def(py::init<const jec::JetAlgo&,
                    const jec::DataType&,
                    const jec::ErrorTypes&,
                    const double>(), 
                    py::arg("algo") = jec::AK4PF,
                    py::arg("type") = jec::DATA,
                    py::arg("errTypes") = jec::kData,
                    py::arg("mu") = 24.68
                    )
      .def("Uncert", &JECUncertainty::Uncert) 
      //.def("", &JECUncertainty::) 
      ;
    m.def("jecUncertainties", jecUncertainties); 
    m.def("jerUncertainties", jerUncertainties); 
