...
if "ifit" in hostname.lower():
    home_dir = "/Users/thothgunter/Documents/WW Prob/Analysis"
elif "swordfish" in hostname.lower():
    if "13TeV" in os.getcwd():
        home_dir = "/home/tgunter/Analysis/WW_analysis/production/Analysis_13TeV"
        page = "13TeV"
        lumi_amount = "35.9"
    if "8TeV" in os.getcwd():
        print("8TeV setup is not operational")
        exit()
...
def bin_df( df, binned_feature, binning_options=binning_options, plotting_options=plotting_options, scales=scales, range=None, bins=None, lumi_scale=1, density=False, weights="weight"):
  """
  ...
  for process in plotting_options.process_decay.unique():
    ...
    if process in unique_df_processes:
      ...
      if weights in df.keys():
        weights_arr = df_process[weights].values
        sq_weights_arr = df_process[weights].values**2
      else:
        weights_arr = None
        sq_weights_arr = None
      ...
      binned_results[process][0] = binned_results[process][0] * scales[process]
      if total == False:
        ...
        binned_results[process][3] = binned_results[process][3] * scales[process]**2.
...
def process_yields( df, df_da=None, scales=scales, yield_style=None ):
    """
    return a dataframe
    """
    if yield_style == None:
        yield_dic = {"Process": [], "Yields": []}
    elif yield_style == "lepton flavor" :
        yield_dic = {"Process": [], "Same Flavor": [], "Diff Flavor": []}
    elif yield_style == "Jets" :
        yield_dic = {"Process": [], "Jet 0": [], "Jet 1": [], "Jet 2": []}

    df_same_sign = df[df.lep1_Charge == df.lep2_Charge]
    df_diff_sign = df[df.lep1_Charge != df.lep2_Charge]

    if type(df_da) == type(None):

        total = 0
        for it, process in enumerate(df_diff_sign.process.unique()):
            yield_dic["Process"].append(process)
            yield_dic["Yields"].append(0)

            for process_decay in df_diff_sign[df_diff_sign.process == process].process_decay.unique():
                yield_dic["Yields"][it] +=  df_diff_sign[df_diff_sign.process_decay == process_decay].weight.sum() * scales[process_decay]
            total += yield_dic["Yields"][it]

        yield_df = pd.DataFrame( yield_dic )
    else:
        df_diff_bins = bin_df(df_diff_sign, "total", scales=scales)
        df_same_bins = bin_df(df_same_sign, "total", scales=scales)
        df_da_diff_bins = bin_df(df_da.query("lep1_Charge != lep2_Charge"), "total", scales=scales)
        df_da_same_bins = bin_df(df_da.query("lep1_Charge == lep2_Charge"), "total", scales=scales)
        ...
        for it, process in enumerate(df_diff_sign.process.unique()):
            ...
            if process == "WJ": 
                same_tot = 0
                for key in df_same_bins:
                    if key not in ["W1JetsToLNu", "W2JetsToLNu", "W3JetsToLNu", "W4JetsToLNu", "plotting"]: 
                        same_tot += df_same_bins[key][0][0]
                fakes = (df_da_same_bins["Da"][0][0] - same_tot) * fake_scalefactor
                yield_dic["Yields"][it] += fakes
                continue
            for process_decay in df_diff_sign[df_diff_sign.process == process].process_decay.unique():
                yield_dic["Yields"][it] += df_diff_bins[process_decay][0][0]
            
            total += yield_dic["Yields"][it]
          
        yield_dic["Process"].append("Total")
        yield_dic["Yields"].append(total)

        yield_dic["Process"].append("Data Total")
        yield_dic["Yields"].append(df_da_diff_bins["Da"][0][0])
